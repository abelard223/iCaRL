...
class iCIFAR100(CIFAR100):
	def __init__(self, root,
				 ...
				 download=False):
		super(iCIFAR100, self).__init__(f'/home/{os.getenv("USER")}/RLLL/data/CIFAR100/raw_data/',
										train=train,
										transform=transform,
	...
	def concatenate(self, datas, labels):
		"""concatenate the datas and labels from list into numpy on axis=0 respectively"""
		con_data = datas[0]
		con_label = labels[0]
	...
	def getTrainData(self, classes, exemplar_set):
		...
		Args:
			classes: [numclass-task_size,numclass]
			exemplar_set: {}
		...
		"""
		datas, labels = [], []
		# re-labels for `1~(s-1)` exemplar sets
		if len(exemplar_set) != 0:
			datas = [exemplar for exemplar in exemplar_set]
			labels = [np.full(len(data), label) for label, data in enumerate(exemplar_set)]

		# re-labels for `s~t` exemplar sets
		s, t = classes
		for label in range(s, t):
			data = self.data[np.array(self.targets) == label]
			datas.append(data)
